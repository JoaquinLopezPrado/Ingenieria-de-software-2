...
import type { Salon } from '@/services/sessionService'
...
const emit = defineEmits<{
  close: []
  submit: [payload: SalonPayload]
}>()

const isEdit = computed(() => !!props.salon)

const form = ref<{ name: string; capacity: number | null }>({
  name: '',
  capacity: null,
})
...
const resetForm = () => {
  form.value = props.salon
    ? { name: props.salon.name, capacity: props.salon.capacity }
    : { name: '', capacity: null }
  error.value = ''
}
...
const handleSubmit = () => {
  ...
  if (!name) {
    error.value = 'Completá el nombre del salón.'
    return
  }
...
<template>
  <div v-if="visible" class="modal-backdrop" @click.self="closeModal">
    <div class="modal-card" role="dialog" aria-modal="true" :aria-label="isEdit ? 'Editar salón' : 'Crear salón'">
      <div class="modal-header">
        <h2>{{ isEdit ? 'Editar salón' : 'Nuevo salón' }}</h2>
      ...
      <form @submit.prevent="handleSubmit" class="modal-form">
        ...
        <div class="form-group">
          <label class="custom-label" for="salon-capacity">Capacidad física</label>
          <input
            id="salon-capacity"
        ...
        <div class="actions">
          ...
          <button type="submit" class="btn-primary" :disabled="loading">
            {{ loading ? 'Guardando...' : isEdit ? 'Guardar cambios' : 'Crear salón' }}
          </button>
        </div>
      </form>
    </div>
